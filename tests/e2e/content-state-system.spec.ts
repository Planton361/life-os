...
test.describe("M1.1A Challenges & Reward Ledger", () => {
  test("M1.1A creates edits corrects abandons archives and reloads challenge progress", async ({ page }) => {
    test.setTimeout(60_000); test.skip(!process.env.PLAYWRIGHT_SUPABASE_AUTH_STATE, "Requires local authenticated Supabase state.");
    const title = uniqueTitle("M1.1A Progress"); const editedTitle = `${title} edited`; const lifecycleTitle = uniqueTitle("M1.1A Lifecycle");
    await openManualPortfolioWithDb(page, "M1.1A requires the local Manual database."); await page.goto("/challenges");
    const main = page.locator("main"); const create = main.getByRole("heading", { name: "Create challenge", exact: true }).locator("xpath=ancestor::section[1]").locator("form");
    await create.locator('input[name="title"]').fill(title); await create.locator('textarea[name="description"]').fill("M1.1A original description"); await create.locator('select[name="periodType"]').selectOption("custom"); await create.locator('input[name="startDate"]').fill("2026-07-01"); await create.locator('input[name="endDate"]').fill("2026-07-31"); await create.locator('input[name="targetValue"]').fill("10"); await create.locator('input[name="unit"]').fill("sessions"); await create.locator('input[name="rewardCoins"]').fill("6"); await create.getByRole("button", { name: "Create challenge" }).click();
    await expect(page.locator("main").getByRole("status")).toHaveText("Challenge created.");
    let active = page.locator("main").getByRole("heading", { name: "Active challenges", exact: true }).locator("xpath=ancestor::section[1]"); let card = active.getByRole("heading", { name: title, exact: true }).locator("xpath=ancestor::article[1]");
    await card.getByText("Edit challenge", { exact: true }).click(); const edit = card.locator("details form"); await edit.locator('input[name="title"]').fill(editedTitle); await edit.locator('textarea[name="description"]').fill("M1.1A edited description"); await edit.locator('select[name="periodType"]').selectOption("monthly"); await edit.getByRole("button", { name: "Save challenge" }).click(); await expect(page.locator("main").getByRole("status")).toHaveText("Challenge updated.");
    active = page.locator("main").getByRole("heading", { name: "Active challenges", exact: true }).locator("xpath=ancestor::section[1]"); card = active.getByRole("heading", { name: editedTitle, exact: true }).locator("xpath=ancestor::article[1]"); let progressForm = card.getByRole("button", { name: "Log progress" }).locator("xpath=ancestor::form[1]"); await progressForm.locator('input[name="increment"]').fill("6"); await progressForm.locator('textarea[name="note"]').fill("First increment"); await progressForm.getByRole("button", { name: "Log progress" }).click();
    card = page.locator("main").getByRole("heading", { name: "Active challenges", exact: true }).locator("xpath=ancestor::section[1]").getByRole("heading", { name: editedTitle, exact: true }).locator("xpath=ancestor::article[1]"); progressForm = card.getByRole("button", { name: "Log progress" }).locator("xpath=ancestor::form[1]"); await progressForm.locator('input[name="increment"]').fill("6"); await progressForm.locator('textarea[name="note"]').fill("Second increment"); await progressForm.getByRole("button", { name: "Log progress" }).click();
    card = page.locator("main").getByRole("heading", { name: "Active challenges", exact: true }).locator("xpath=ancestor::section[1]").getByRole("heading", { name: editedTitle, exact: true }).locator("xpath=ancestor::article[1]"); await expect(card).toContainText("12 / 10 sessions"); await expect(card).toContainText("overachieved"); await card.getByText("Correct latest progress", { exact: true }).click(); const correction = card.getByRole("button", { name: "Save correction" }).locator("xpath=ancestor::form[1]"); await correction.locator('input[name="increment"]').fill("7"); await correction.locator('textarea[name="note"]').fill("Corrected latest increment"); await correction.getByRole("button", { name: "Save correction" }).click(); await page.reload();
    card = page.locator("main").getByRole("heading", { name: "Active challenges", exact: true }).locator("xpath=ancestor::section[1]").getByRole("heading", { name: editedTitle, exact: true }).locator("xpath=ancestor::article[1]"); await expect(card).toContainText("13 / 10 sessions"); await expect(card).toContainText("monthly");
    const freshCreate = page.locator("main").getByRole("heading", { name: "Create challenge", exact: true }).locator("xpath=ancestor::section[1]").locator("form"); await freshCreate.locator('input[name="title"]').fill(lifecycleTitle); await freshCreate.locator('input[name="targetValue"]').fill("1"); await freshCreate.locator('input[name="unit"]').fill("item"); await freshCreate.locator('input[name="rewardCoins"]').fill("1"); await freshCreate.getByRole("button", { name: "Create challenge" }).click();
    const lifecycleCard = page.locator("main").getByRole("heading", { name: "Active challenges", exact: true }).locator("xpath=ancestor::section[1]").getByRole("heading", { name: lifecycleTitle, exact: true }).locator("xpath=ancestor::article[1]"); await lifecycleCard.getByText("Edit challenge", { exact: true }).click(); await lifecycleCard.getByRole("button", { name: "Abandon" }).click();
    let history = page.locator("main").getByRole("heading", { name: "Challenge history", exact: true }).locator("xpath=ancestor::section[1]"); let historicalCard = history.getByRole("heading", { name: lifecycleTitle, exact: true }).locator("xpath=ancestor::article[1]"); await expect(historicalCard).toContainText("abandoned"); await historicalCard.getByRole("button", { name: "Archive challenge" }).click(); await page.reload();
    history = page.locator("main").getByRole("heading", { name: "Challenge history", exact: true }).locator("xpath=ancestor::section[1]"); historicalCard = history.getByRole("heading", { name: lifecycleTitle, exact: true }).locator("xpath=ancestor::article[1]"); await expect(historicalCard).toContainText("archived"); await expect(historicalCard.locator("form")).toHaveCount(0); await expect(historicalCard.getByRole("button", { name: /Complete|Log progress|Save challenge/ })).toHaveCount(0);
  });

  test("M1.1A completes challenge credits reward once and reloads ledger", async ({ page }) => {
    test.setTimeout(60_000); test.skip(!process.env.PLAYWRIGHT_SUPABASE_AUTH_STATE, "Requires local authenticated Supabase state."); const title = uniqueTitle("M1.1A Reward"); const reward = 7;
    await openManualPortfolioWithDb(page, "M1.1A requires the local Manual database."); await page.goto("/challenges"); const main = page.locator("main");
    const balanceBlock = main.getByText("Reward balance", { exact: true }).locator("xpath=.."); const initialText = await balanceBlock.locator("p").filter({ hasText: /coins$/ }).textContent(); const initialBalance = Number(initialText?.match(/-?\d+/)?.[0] ?? 0);
    const create = main.getByRole("heading", { name: "Create challenge", exact: true }).locator("xpath=ancestor::section[1]").locator("form"); await create.locator('input[name="title"]').fill(title); await create.locator('textarea[name="description"]').fill("M1.1A atomic reward proof"); await create.locator('select[name="periodType"]').selectOption("daily"); await create.locator('input[name="targetValue"]').fill("5"); await create.locator('input[name="unit"]').fill("steps"); await create.locator('input[name="rewardCoins"]').fill(String(reward)); await create.getByRole("button", { name: "Create challenge" }).click();
    let active = page.locator("main").getByRole("heading", { name: "Active challenges", exact: true }).locator("xpath=ancestor::section[1]"); let card = active.getByRole("heading", { name: title, exact: true }).locator("xpath=ancestor::article[1]"); await expect(card.getByRole("button", { name: "Complete challenge" })).toHaveCount(0); await expect(card).toContainText("Completion unlocks");
    const progress = card.getByRole("button", { name: "Log progress" }).locator("xpath=ancestor::form[1]"); await progress.locator('input[name="increment"]').fill("5"); await progress.locator('textarea[name="note"]').fill("Target reached explicitly"); await progress.getByRole("button", { name: "Log progress" }).click();
    active = page.locator("main").getByRole("heading", { name: "Active challenges", exact: true }).locator("xpath=ancestor::section[1]"); card = active.getByRole("heading", { name: title, exact: true }).locator("xpath=ancestor::article[1]"); await card.getByRole("button", { name: "Complete challenge" }).click(); await expect(page.locator("main").getByRole("status")).toHaveText("Challenge completed and reward credited once.");
    let history = page.locator("main").getByRole("heading", { name: "Challenge history", exact: true }).locator("xpath=ancestor::section[1]"); let completed = history.getByRole("heading", { name: title, exact: true }).locator("xpath=ancestor::article[1]"); await expect(completed).toContainText("completed"); await expect(completed).toContainText("5 / 5 steps"); await expect(completed.getByRole("button", { name: "Log progress" })).toHaveCount(0);
    let ledger = page.locator("main").getByRole("heading", { name: "Reward ledger", exact: true }).locator("xpath=ancestor::section[1]"); let entry = ledger.locator("article").filter({ hasText: `Challenge reward: ${title}` }); await expect(entry).toHaveCount(1); await expect(entry).toContainText(`+${reward} coins`); await expect(page.locator("main").getByText("Reward balance", { exact: true }).locator("xpath=..").locator("p").filter({ hasText: /coins$/ })).toHaveText(`${initialBalance + reward} coins`); await page.reload();
    history = page.locator("main").getByRole("heading", { name: "Challenge history", exact: true }).locator("xpath=ancestor::section[1]"); completed = history.getByRole("heading", { name: title, exact: true }).locator("xpath=ancestor::article[1]"); await expect(completed).toContainText("5 / 5 steps"); await completed.getByRole("button", { name: "Complete challenge" }).click(); await expect(page.locator("main").getByRole("status")).toHaveText("Challenge completed and reward credited once."); await page.reload();
    ledger = page.locator("main").getByRole("heading", { name: "Reward ledger", exact: true }).locator("xpath=ancestor::section[1]"); entry = ledger.locator("article").filter({ hasText: `Challenge reward: ${title}` }); await expect(entry).toHaveCount(1); await expect(page.locator("main").getByText("Reward balance", { exact: true }).locator("xpath=..").locator("p").filter({ hasText: /coins$/ })).toHaveText(`${initialBalance + reward} coins`); completed = page.locator("main").getByRole("heading", { name: "Challenge history", exact: true }).locator("xpath=ancestor::section[1]").getByRole("heading", { name: title, exact: true }).locator("xpath=ancestor::article[1]"); await expect(completed.locator("form").filter({ has: completed.getByRole("button", { name: "Log progress" }) })).toHaveCount(0);
  });
});
