    ...
    >
      ...
      <div className="grid gap-3 p-4">
        {Array.from({ length: rows }).map((_, index) => (
          <SkeletonLine
            className={index === rows - 1 ? "h-10 w-32 rounded-[12px]" : "h-4 w-full"}
            key={index}
          />
        ))}
      </div>
...
export default function CodingLoading() {
  return (
    <div
      aria-busy="true"
      aria-label="Loading coding overview"
    ...
    >
      <header className="overflow-hidden rounded-[18px] border border-[var(--border-subtle)] bg-[rgba(15,23,36,.72)] shadow-[0_8px_22px_rgba(0,0,0,.12)]">
        <div className="grid gap-3 bg-[linear-gradient(90deg,rgba(91,124,250,.055),transparent_46%)] px-4 py-4 xl:grid-cols-[minmax(0,1fr)_auto] xl:items-center">
          <div>
          ...
          <div className="flex flex-wrap gap-2 xl:justify-end">
            ...
            <SkeletonLine className="h-11 w-48 rounded-[12px]" />
          </div>
        </div>
      </header>

      <div className="grid gap-2 xl:grid-cols-12">
        <SkeletonPanel className="xl:col-span-7" rows={5} />
        <SkeletonPanel className="xl:col-span-5" rows={4} />
        <SkeletonPanel className="xl:col-span-4" rows={4} />
        <SkeletonPanel className="xl:col-span-5" rows={5} />
        <SkeletonPanel className="xl:col-span-3" rows={4} />
