...
export function HabitTrackingPage({
  ...
  return (
    <main className={healthPage} data-health-detail="habits">
      ...
      <div className="health-workspace habits-workspace grid min-w-0 gap-4">
        <HealthSection title="Habits & Verlauf" className="habit-list">
                ...
                >
                  ...
                  >
                    ...
                    <p className="mt-2 text-xs text-[var(--text-muted)]">
                      7 Tage:{" "}
                      {number(days.slice(-7).reduce((s, d) => s + d.value, 0))}{" "}
                      {h.unit ?? ""} · 30 Tage:{" "}
          ...
          {!snapshot?.habits.length && (
            <p className={healthMuted}>
              Noch keine Habits. Neue Habits und tägliche Einträge findest du im
              Dashboard.
            </p>
          )}
        </HealthSection>
        <HealthSection
          className="habit-analytics"
          ...
          {habit && snapshot ? (
            <div data-habits-section="selected">
              ...
              >
                {[
                  ...
                  <Link
                    ...
                    aria-current={
                      (count === 1 ? "day" : count === 7 ? "week" : "month") ===
                      value
                        ? "page"
                        : undefined
                    }
              ...
              <p className="mt-3 text-xs text-[var(--text-muted)]">
                Farbe zeigt Aktivität. Frühere Ziele sind nicht gespeichert; der
                Zielvergleich verwendet das aktuelle Tagesziel.
              </p>
            </div>
          ) : (
            <p className={healthMuted}>
        ...
        <HealthSection title="Habit verwalten" className="habit-management">
          {habit && !habit.archivedAt && data.canWrite ? (
            ...
            <p className={healthMuted}>
              {habit?.archivedAt
                ? "Archivierte Habits bleiben im Verlauf."
                : "Wähle ein aktives Habit zum Bearbeiten."}
            </p>
          )}
        </HealthSection>
        <HealthSection title="Tageszeiten" className="habit-windows">
