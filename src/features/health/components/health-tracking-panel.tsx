...
export function SleepTrackingPanel({ data, status }: { data: HealthTrackingData; status?: string }) {
  ...
  return <section aria-labelledby="sleep-tracking-title" className="rounded-[18px] border border-[var(--border-subtle)] bg-[rgba(15,23,36,.86)] p-4">
    <h2 id="sleep-tracking-title" className="text-lg font-semibold text-[var(--text-primary)]">Schlafprotokoll</h2>
    <p className="mt-1 text-xs text-[var(--text-secondary)]">Dauer und optionale eigene Einschätzung. Ein Datum erneut speichern, um den Eintrag zu korrigieren.</p>
    {status ? <p role={status === "saved" ? "status" : "alert"} className="mt-2 text-xs text-[var(--accent-cyan)]">{status === "saved" ? "Schlafeintrag gespeichert." : status === "blocked" ? "Lokale Anmeldung erforderlich." : "Schlafeintrag konnte nicht gespeichert werden."}</p> : null}
    {data.profileId === "manual" && data.authAvailable ? <form action={saveSleepAction} className="mt-3 grid gap-2 sm:grid-cols-2">
      <label className="grid gap-1 text-xs text-[var(--text-secondary)]">Nacht / Datum<input required name="sleepDate" type="date" defaultValue={entries[0]?.sleepDate} className={inputClass}/></label>
      <label className="grid gap-1 text-xs text-[var(--text-secondary)]">Stunden<input required name="hours" type="number" min="0" max="24" defaultValue={entries[0] ? Math.floor(entries[0].durationMinutes / 60) : 7} className={inputClass}/></label>
      <label className="grid gap-1 text-xs text-[var(--text-secondary)]">Minuten<input required name="minutes" type="number" min="0" max="59" defaultValue={entries[0]?.durationMinutes ? entries[0].durationMinutes % 60 : 30} className={inputClass}/></label>
      <label className="grid gap-1 text-xs text-[var(--text-secondary)]">Qualität (optional)<select name="quality" defaultValue={entries[0]?.quality ?? ""} className={inputClass}><option value="">Ohne Angabe</option>{[1,2,3,4,5].map(v=><option key={v}>{v}</option>)}</select></label>
      <button className={`${buttonClass} self-end`} type="submit">Schlaf speichern</button>
      <label className="grid gap-1 text-xs text-[var(--text-secondary)] sm:col-span-2">Notiz (optional)<input name="note" maxLength={500} defaultValue={entries[0]?.note ?? ""} className={inputClass}/></label>
    </form> : <p className="mt-3 text-xs text-[var(--text-muted)]">{data.profileId === "manual" ? "Melde dich an, um Schlaf zu erfassen." : "In diesem Profil sind keine Einträge möglich."}</p>}
    <div aria-label="Schlafverlauf" className="mt-3 grid gap-1">{entries.length ? entries.slice(0, 14).map(e=><div key={e.id} className="flex justify-between rounded-lg border border-[var(--border-subtle)] px-2 py-1.5 text-xs"><span>{e.sleepDate}</span><span>{sleepLabel(e.durationMinutes)}{e.quality ? ` · Einschätzung ${e.quality}/5` : " · ohne Qualitätseinschätzung"}</span></div>) : <p className="text-xs text-[var(--text-muted)]">Noch keine Schlafeinträge.</p>}</div>
  </section>;
}
