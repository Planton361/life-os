...
async function canonicalApiPort() {
  const result = await run("docker", ["inspect", `supabase_kong_${CANONICAL_TARGET_PROJECT}`]);
...
async function targetPublicRuntimeEnv() {
  ...
  try {
    const status = await run("pnpm", ["exec", "supabase", "status", "--workdir", statusWorkdir, "--output", "env"]);
    await assertSuccessful(status, "Canonical Target status lookup");
    const values = parseSupabaseEnv(status.stdout);
    const publishableKey = values.PUBLISHABLE_KEY ?? values.ANON_KEY;
    if (!values.API_URL || !publishableKey) throw new Error("RUNTIME_GUARD_CANONICAL_TARGET_PUBLIC_CONFIG_UNAVAILABLE");
    return {
