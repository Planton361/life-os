...
| Resource | Rohmaterial wird Resource oder Resource-Link. | Raw Capture -> Resource Draft -> Resource erstellen -> Resources | Resource Draft ist verbunden: Titel, Typ, Kurzfassung/Inhalt, optionale URL. Resource Create + Inbox Resolve laufen seit R1.6.8B transaktional; das Inbox Item wird ohne Hard Delete archiviert. | Keine Resource-Graph-Kanten automatisch erzeugen. |
...
| Resource Draft | Aus Capture entsteht ein Resource Draft mit Typ, Quelle, Inhalt und Review-Status. | Minimal verbunden: Resource wird aus bestaetigtem Draft gespeichert und in `/resources` sichtbar. |
...
| Resource Draft | `title`, `body/url`, `type`, `area`, `source`, `privacy`, `status`, `reviewNeeded`, optionale Relation | Minimaler Inbox-Create-Pfad schreibt `resources.title`, `type`, `summary`, `url`, `source` und `review_needed`. | Direkte `source_inbox_item_id`, Resource-Relationen, Privacy/Status-Spalten und Graph-Anbindung fehlen. | Resources, Resource Graph / Knowledge Network | Keine Migration in R1.6.7; fehlende Felder bleiben Auditpunkt. |
...
| Resource | Route sichtbar und minimal verbunden. | Resource Draft erstellt eine echte Resource; Attach to existing bleibt vorbereitet. | Direkte Inbox-Resource-FK, Resource Picker und Graph Entry fehlen. | R1.6.7 |
