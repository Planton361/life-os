...
| Goal outcome planning | `CONNECTED_GAP` | PP1 #23 Slice 1 plus Issue #41 implement the accepted #39/#40 Goal Journey: read/work-first Goal detail, exactly one unarchived Current Milestone enforced by a partial unique index and serialized atomic switches, pure one-action guidance based only on canonical Goal state and Task-to-Task dependencies, 68/32 Current Work/Journey layout, explicit Planung bearbeiten/Fertig mode, quiet execution-mode DoD end gate with criterion detail only in planning/final review, action-oriented JETZT headings, compact empty current work and planning-only unassigned Goal Tasks (acceptance repair #41 comment 5844794278), current-only contextual title-first Task creation, append-only Milestone Review with atomic next-planned advancement, and final Definition-of-Done Goal Review. `tests/e2e/issue-41-goal-journey.spec.ts` proves authenticated Manual creation, reviews, task completion without implicit achievement, reload, keyboard/focus, responsive bounds and 3840/1920/mobile screenshots; `tests/supabase/pp1-goal-current-milestone.sql` and `tests/supabase/pp1-goal-current-milestone-concurrency.mjs` prove the database invariant, history retention, atomic advancement and concurrent switches. Existing PP1 correction, evidence and history ledgers remain append-only; Goal/Etappe revisions, legacy snapshots and episode-aware projections remain preserved. | Slice 2 remains deferred: Task completion episodes, Project/Project-Milestone completion history, immutable Skill Evidence revisions and Skill Evidence as Goal decision evidence; preserve legacy progress fields and no generic percentage |
