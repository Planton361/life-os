...
| Goal outcome planning | `CONNECTED_GAP` | PP1 #23 Slice 1: lightweight/read-first Goal detail with description/why, Weg zum Ziel Etappen, human-readable criteria and explicit Goal Review; append-only Goal/Etappe/Criterion revisions, corrections/reopen episodes, decision-specific canonical evidence refs, exact achievement basis/outcome history, atomic contextual Project/Task creation, canonical next-step cue and honest Today/Portfolio/Dashboard projections. `tests/e2e/pp1-goal-outcome.spec.ts` and `tests/e2e/pp1-integrated-goal-path.spec.ts` prove Manual reload/browser/surface behavior; DB boundary proofs are `tests/supabase/pp1-goal-outcome-repairs.sql` and `tests/supabase/pp1-integrated-goal-path.sql` | Slice 2 remains explicitly deferred: Task completion episodes, Project/Project-Milestone completion history, immutable Skill Evidence revisions and Skill Evidence as Goal decision evidence; preserve legacy progress fields and no generic percentage |
