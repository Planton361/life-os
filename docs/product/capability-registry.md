...
| Goal linked projects/tasks | `CONNECTED` | goal relations plus atomic Goal/Etappe contextual Project/Task creation and canonical next-step read projection; #23 Slice 1 browser proof | preserve explicit Goal/Etappe context; Task/Project completion episodes remain Slice 2 |
...
| Goal outcome planning | `CONNECTED_GAP` | PP1 #23 Slice 1: lightweight/read-first Goal detail with description/why, Weg zum Ziel Etappen, human-readable criteria and a composed planning surface with contextual Ergebnis prüfen action; append-only Goal/Etappe/Criterion revisions, corrections/reopen episodes, decision-specific canonical evidence refs, exact achievement basis/outcome history, atomic contextual Project/Task creation, canonical R2-10 next-step readiness cue and honest Today/Portfolio/Dashboard projections. Management forms are contextual disclosures with focus return and reload proof; legacy Goal/Etappe markers preserve unknown historical identity/timing without synthetic `updated_at` evidence; accepted event amendments and evidence attach/replace/withdraw/retrospective supplement keep exact superseded references and reasons. Current achieved Goal/Etappe projections select the open episode and its latest effective assertion; amendment-only changes aggregate active evidence across that episode's correction chain without rewriting event-local rows, while older-episode corrections remain historical. `tests/e2e/pp1-goal-follow-up.spec.ts`, `tests/e2e/pp1-goal-outcome.spec.ts` and `tests/e2e/pp1-integrated-goal-path.spec.ts` prove lightweight capture, contextual fresh-draft guidance, blocker release, episode-aware current outcome, evidence retention/replacement/withdrawal, composed 4K/1920/mobile planning bounds, keyboard disclosure behavior and Manual reload/browser/surface behavior; focused domain/repository tests cover readiness, amendment basis and achieved-state boundaries; DB boundary proofs are `tests/supabase/pp1-goal-outcome-repairs.sql`, `tests/supabase/pp1-integrated-goal-path.sql` and `tests/supabase/pp1-goal-history-ledger.sql` | Slice 2 remains explicitly deferred: Task completion episodes, Project/Project-Milestone completion history, immutable Skill Evidence revisions and Skill Evidence as Goal decision evidence; preserve legacy progress fields and no generic percentage |
