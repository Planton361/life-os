...
| Goal outcome planning | `CONNECTED_GAP` | PP1 #23 Slice 1 and Issue #41 preserve the accepted #39/#40 semantics: one Current Milestone enforced by a partial unique index and serialized atomic switches; pure one-action guidance based on canonical Goal state and Task-to-Task dependencies; current-only contextual title-first Task creation; append-only Milestone Review with atomic next-planned advancement; explicit Goal review, with no automatic achievement from Task completion. Issue #46 implements the USER ACCEPTED #44 surface: distinct Goal identity, one shared Current Work area for the current milestone, guidance, primary action and current Tasks, V5 navy surface, question-led progressive planning, and a quiet roadmap showing only milestone state/title and Goal review without task repetition, counts or generic progress. `tests/e2e/issue-46-simplified-goal-surface.spec.ts` proves the authenticated Manual planning flow, real READY/BLOCKED task dependency, explicit milestone review, reload, keyboard/focus, responsive bounds and 3840×2160 / 1920×1080 / mobile screenshots; `tests/e2e/issue-41-goal-journey.spec.ts` retains the task-completion, final Goal review, explicit achievement/reopen and reload regression. `tests/supabase/pp1-goal-current-milestone.sql` and `tests/supabase/pp1-goal-current-milestone-concurrency.mjs` prove the database invariant, history retention, atomic advancement and concurrent switches. Existing PP1 correction, evidence and history ledgers remain append-only; Goal/Etappe revisions, legacy snapshots and episode-aware projections remain preserved. | Slice 2 remains deferred: Task completion episodes, Project/Project-Milestone completion history, immutable Skill Evidence revisions and Skill Evidence as Goal decision evidence; preserve legacy progress fields and no generic percentage |
